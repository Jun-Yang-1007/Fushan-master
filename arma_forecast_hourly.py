...
from database.dts_env_repository import getSiteData,save_daily_forecast_data,save_hour_forecast_data
from utils import dateshift_Day,dateshift_Hour
...
TODAY = '2020-08-10 10:00:00'
#TODAY=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
# 前一天
YESTERDAY = dateshift_Hour(TODAY,-1)       #today + -1
...
FORECAST_END_DATETIME = dateshift_Hour(TODAY,RUN_LEN-1)          #预测起始当天开始————目标当天-1
...
def arma_forecast(dataset,column):                                          
    datas = arma.forecast(dataset,delta=7, freq='H')       #delta=3
    if datas is None:
        return datas
    values = datas.values
    index = datas.index
...
if __name__ == '__main__':
    ...
    # while today <= datetime.datetime.strptime("2020-09-20 00:00:00", "%Y-%m-%d %H:%M:%S"):
    #     FORECAST_START_DATETIME = datetime.datetime.strftime(today, "%Y-%m-%d %H:%M:%S")
        ...
        for site in sites:
            for factor in factors:
                ...
                if arma_datas is not None:
                    ...
                    save_hour_forecast_data(FORECAST_START_DATETIME,site['id'],arma_datas.values,factor['code'],model="ARMA")
